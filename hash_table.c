...
static HashTableEntry* createHashTableEntry(unsigned int key, void* value) {
  ...
  // set the key as the param key
  node -> key = key;
  // set the value as the param value
  node -> value = value;
...
static HashTableEntry* findItem(HashTable* hashTable, unsigned int key) {
  ...
  // point to the bucket
  HashTableEntry** bucket = hashTable -> buckets;
  // point to the first item in the bucket
  HashTableEntry* node = bucket[index];
...
void destroyHashTable(HashTable* hashTable) {
  ...
  HashTableEntry* node;
  HashTableEntry* temp;
  // loop through all the num_buckets
  for (int i = 0; i < hashTable->num_buckets; i++) {
    bucket = hashTable->buckets;
    // loop through the nodes in the buckets
    node = bucket[i];
    while(node) {
      // free the node's value and the node
      temp = node;
      node = node->next;
      free(temp->value);
      free(temp);
  ...
  // free the HashTable buckets and HashTable
...
void* insertItem(HashTable* hashTable, unsigned int key, void* value) {
  ...
  // point to the bucket
  HashTableEntry** bucket = hashTable->buckets;
  // if the bucket's first entry is NULL
  if(!bucket[index]) {
    // create a new node
    HashTableEntry* node = createHashTableEntry(key, value);
    // place it in the empty bucket
    bucket[index] = node;
    // return NULL
    return NULL;
  }
  // else, attempt to find the item
  HashTableEntry* node = findItem(hashTable, key);
  HashTableEntry* temp;
  // if the node is not present (NULL)
  if(!node) {
    // loop to end of list
    node = bucket[index];
    while(node) {
    ...
    // create new entry at the end of the list
    temp -> next = createHashTableEntry(key, value);
    // return NULL
    return NULL;
